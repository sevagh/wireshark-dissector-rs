...
quick_main!(|| -> Result<i32> {
    let fd: Result<RawFd> = (|| {
    let fd = socket::socket(socket::AddressFamily::Inet,
                            socket::SockType::Stream,
                            socket::SockFlag::empty(),
                            socket::SOL_TCP)?;
        let address = SOCK.to_socket_addrs()?.next()
            .expect("Cannot convert bind to socket address");
        socket::bind(fd, &socket::SockAddr::new_inet(socket::InetAddr::from_std(&address)))?;
    ...
    loop {
        let nread = unsafe {
            recv(fd_ , payload.as_mut_ptr() as *mut c_void, MSGLEN, MSG_WAITALL)
        };
        if nread == 0 {
